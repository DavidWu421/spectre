...
SPECTRE_TEST_CASE("Unit.PointwiseFunctions.AnalyticSolutions.Gr.SphKerrSchild",
  ...
  tnsr::ii<DataVector, 4, Frame::Inertial> eta_bar{used_for_size};
  eta_bar.get(0, 0) = -1.;
  for (size_t k = 0; k < 3; ++k) {
    ...
    for (size_t m = k; m < 3; ++m) {
      for (size_t i = 0; i < 3; ++i) {
        eta_bar.get(k + 1, m + 1) += jacobian.get(k, i) * jacobian.get(m, i);
      }
    }
  }

  std::cout << "This is eta_bar: "
            << "\n"
            << std::setprecision(12) << eta_bar << "\n";
  ...
  // std::cout << "This is deriv_lapse" << std::setprecision(12) << deriv_lapse
  //           << "\n";

  // PSI TEST
  auto spacetime_metric_psi =
      make_with_value<tnsr::aa<DataVector, 3, Frame::Inertial>>(H, 0.);
  spacetime_metric_psi.get(0, 0) = -square(get(lapse));

  for (size_t m = 0; m < 3; ++m) {
    spacetime_metric_psi.get(0, 0) +=
  ...
  for (size_t i = 0; i < 3; ++i) {
    spacetime_metric_psi.get(0, i + 1) = 0.;
    for (size_t m = 0; m < 3; ++m) {
      spacetime_metric_psi.get(0, i + 1) +=
          spatial_metric.get(m, i) * shift.get(m);
    }
    for (size_t j = i; j < 3; ++j) {
      spacetime_metric_psi.get(i + 1, j + 1) = spatial_metric.get(i, j);
    }
  }

  // std::cout << "This is spacetime_metric_psi:\n"
  //           << std::setprecision(12) << spacetime_metric_psi << "\n";

  // PHI TEST
  auto phi = make_with_value<tnsr::iaa<DataVector, 3, Frame::Inertial>>(H, 0.);
  for (size_t k = 0; k < 3; ++k) {
